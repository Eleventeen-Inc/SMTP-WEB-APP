...
import { BubbleChatQuestionIcon, Chart02Icon, ComputerTerminal01Icon, FolderLibraryIcon, Globe02Icon, Layers01Icon, LockKeyIcon, MailOpenIcon, SettingDone02Icon, WebhookIcon } from '@hugeicons/core-free-icons';
...
const navigationSettings = [
    {
        title: "Settings",
        url: "/app/settings",
        icon: SettingDone02Icon,
        soon: false
    },
    {
        title: "Help Center",
        url: "help.eleventeen.net",
...
export default function ApplicationSidebar() {
    ...
    return (
        <Sidebar className='border-r-0'>
            <SidebarHeader>
                <div className='p-2.5'>
                    <svg width="105" height="14" viewBox="0 0 661 88" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <SidebarContent className='space-y-4'>
                <SidebarGroup>
                    <SidebarGroupLabel className='text-sm font-semibold text-muted-foreground'>Application</SidebarGroupLabel>
                    <SidebarGroupContent>
                        <SidebarMenu className='space-y-1'>
                            {navigationAppMenu.map((item, index) => (
                                <SidebarMenuItem key={index}>
                                    <SidebarMenuButton disabled={item.soon} className={`font-semibold ${pathname === item.url ? "text-foreground" : "text-muted-foreground"
                                        } ${item.soon ? "pointer-events-none opacity-50" : ""}`} asChild isActive={pathname === item.url}>
                                        <a href={item.url}>
                ...
                <SidebarGroup>
                    <SidebarGroupLabel className='text-sm font-semibold text-muted-foreground'>Developer</SidebarGroupLabel>
                    <SidebarGroupContent>
                        <SidebarMenu className='space-y-1'>
                            {navigationDevMenu.map((item, index) => (
                                <SidebarMenuItem key={index}>
                                    <SidebarMenuButton className={`font-semibold ${pathname === item.url ? "text-foreground" : "text-muted-foreground"
                                        } ${item.soon ? "pointer-events-none opacity-50" : ""}`} asChild isActive={pathname === item.url}>
                                        <a href={item.url}>
                                            <HugeiconsIcon icon={item.icon} strokeWidth={2.2} />
            ...
            <SidebarFooter>
                <SidebarGroup>
                    <SidebarGroupLabel className='text-sm font-semibold text-muted-foreground'>Settings</SidebarGroupLabel>
                    <SidebarGroupContent>
                        <SidebarMenu className='space-y-1'>
